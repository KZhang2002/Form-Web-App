...
export const Login = () => {
  ...
  const handleLogin = async () => {
    ...
    setLoading(true);
    try {
      const response = await axios.get('http://localhost:8080/user/login/user', {
        params: {
      ...
      // If the login is successful, store user info and navigate
      if (response.status === 200) {
        setLoading(false);
        navigate("/home", {state: response.data}); // Pass the response data (user info) to the next page
      }
    } catch (error) {
      setLoading(false);
